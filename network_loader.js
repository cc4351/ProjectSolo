...
*/
for(i = 0; i<connect[0].length; i++){
    var pt1, pt2, x1, x2, y1, y2, len, tmp;
    pt1 = nodeData[connect[0][i].from];
    pt2 = nodeData[connect[0][i].to];
    x1 = pt1.x;
    y1 = pt1.y;
    x2 = pt2.x;
    y2 = pt2.y;
    ...
    tmp = {from: "n"+connect[0][i].from, to: "n"+connect[0][i].to, x1: x1, y1: y1, x2: x2, y2: y2, len: len, id: "l"+i};
    console.log(tmp);
    lineData.push(tmp);
...
//				var translate2 = 'translate(' + 0 + ',' + 0 + ')';
    lines.each(function(){
        ...
        var data1 = d3.select("#n" + connect[0][idx]["from"]);
        var data2 = d3.select("#n" + connect[0][idx]["to"]);
...
nodes.each(function(){
    d3.select(this).on("click", function(){
        ...
        if(data.hasOwnProperty("link")){
            var newColor = this.style.fill == "rgb(255, 0, 0)"? "rgb(100, 100, 100)":"rgb(255, 0, 0)";
