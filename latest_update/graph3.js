...
var render = function() {
  if (scrollTop != newScrollTop) {
	...
	function introScale(svgC, top, max=0){
		...
		nodes0.each(function(){
			if(scrollTop >= d3.select(this).attr("tol")){
				d3.select(this).attr("opacity", 0.8).attr("fill", "#eb2226");
			}
			if(scrollTop < d3.select(this).attr("tol")){
	...
	function rescale(lines=[], connect=[], svgC, sumLen = 0, top){
		...
		//color the two designated dots blue if the graph is currently in range
		//otherwise color them red
		if(scrollTop - top>scroll_length && scrollTop <bottom){
			bluePts.forEach(ele => {
				var oriColor = d3.select("#"+header+ele).attr("fill");
				if(oriColor != nodeGray){d3.select("#"+header+ele).attr("fill", nodeBlue);}
			...
			bluePts.forEach(ele => {
				var oriColor = d3.select("#"+header+ele).attr("fill");
				if(oriColor != nodeGray){d3.select("#"+header+ele).attr("fill", "#eb2226");}
...
function click(nodes, nData){
	...
	nodes.each(function(){
		d3.select(this).on("click", function(){
			...
			data = nData[ind];
			//color alternating between blue/red and gray
			if(bluePts.includes(ind) && d3.select(this).attr("bFlag") == 1){
				var newColor = d3.select(this).attr("fill") == nodeBlue? nodeGray:nodeBlue;
			}else{
				var newColor = d3.select(this).attr("fill") == "#eb2226"? nodeGray:"#eb2226";
			}
