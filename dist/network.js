...
//see https://stackoverflow.com/questions/37443482/what-is-the-difference-between-window-innerwidth-and-screen-width for difference between window.innerWidth vs screen.width
			...
			var svgContainer = d3.select("#network_graph").append("svg")
							 .attr("width", 300)
							 .attr("height", 600);
...
//			console.log("length" + lineData.length)
			...
			var SCROLL_LENGTH = network_na.scrollHeight - HEIGHT;//scollable distance
			var scrollTop = 0;
    		var newScrollTop = 0;
			...
			d3.select("#network_na").on("scroll.scroller", function() {
				newScrollTop = d3.select("#network_na").node().scrollTop;//get the latest scrollTop
				});
